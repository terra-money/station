...
export function nsecEncode(hex: RawKey): `nsec1${string}` {
  return encodeBytes("nsec", hex.privateKey.toString())
}

export function npubEncode(hex: RawKey): `npub1${string}` {
  return encodeBytes("npub", hex.getSchnorrPubKey().toString())
}
